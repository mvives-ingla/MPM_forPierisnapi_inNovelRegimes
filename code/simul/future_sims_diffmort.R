...
## Simulations with different predatory pressures
...
## Probability that a thermal extreme event occurs
...
sims_future <- expand_grid(site = c("lowl", "mide"),
  ...
  mutate(prob_drought = case_when(site == "mide" | period == "g1g2" ~ 0,
                                  period == "g3g4" ~ 1,
                                  T ~ prob_drought),
         pred_factor = .92) %>% # Change the predatory pressure simulated here
  nest(sim_cond = -sim_set) %>% 
  mutate(simul = map(sim_cond,
                     ~ simul_scenario(nsims = 5000,
                                      ndays = 70,
                                      broods = .$period,
                                      data = data_list,
                                      prob_35 = .$prob35,
                                      ...
                                      rob_test = .$pred_factor))) %>% 
  unnest(simul)




save(sims_future, file = paste0("data/mpm/sims_output/boot/sims_future_drought_",
