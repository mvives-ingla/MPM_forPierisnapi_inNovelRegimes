...
# Simulations without microclimatic buffering effects
...
setwd("~/maria_analyses/mpm_pnapi/")
...
## Probability that a thermal extreme event occurs at a MACROCLIMATIC scale
prob_extreme <- read_csv("data/clima/future_extremes_prob_allparams.csv") %>%
  select(TX, site, period, starts_with("macro_prop")) %>%
  pivot_longer(cols = starts_with("macro_prop"),
               values_to = "prob_occ",
               names_to = "GWL",
               names_prefix = "macro_prop_") %>%
  pivot_wider(id_cols = c(site, period, GWL),
              names_from = TX,
              names_prefix = "prob",
              values_from = prob_occ) %>%
  mutate(GWL = as.double(GWL),
         GWL = if_else(GWL == .85, 1, GWL))
...
sims_imp0_macro <- expand_grid(site = c("lowl", "mide"),
...
save(sims_imp0_macro, file = paste0("data/mpm/sims_output/boot/sims_future_drought_",
