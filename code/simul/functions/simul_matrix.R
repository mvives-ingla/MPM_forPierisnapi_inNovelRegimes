# Builds a bootstrapped transition matrix from raw data (fixed longevitity)
# INPUT:
...
simul_matrix <- function(data_juv, data_repr, data_ad, brood_period) {
  ...
  if (brood_period == "g1g2") {
    ...
    ad_long <- 23 # maximum adult longevity (taken from adult fecundity data)
    juv_long <- 40 # maximum juvenile longevity (taken from juvenile data)
