...
# Setting the minimum age for pupal eclosion at age 8 days
...
build_matrix <- function(juvsurvs, juvecls, adfecs, adsurvs) {
  ...
  for (i in 1:(m+a)) {
    if (i == 1) {
      ...
      A[i, 8:m] <- juvsurvs[8:m]*juvecls[8:m]
      # vector of pupal eclosion (row m+1, juvenile columns)
      # survival*probabilty of eclosion, imposing a 0 prob for pupal eclosion to individuals younger than 8 days
    } else {
      if (length(adsurvs) == 1) {
        A[i, i-1] <- adsurvs
